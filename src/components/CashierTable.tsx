...
import { GetCashierdashboard } from "../services/Axios";
...
export default function CashierTable() {
  const [cashier, setCashier] = useState<Props[]>([]);
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        const response = await GetCashierdashboard(1, 10);
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow w-full overflow-x-auto">
      <h3 className="font-semibold text-lg mb-4">Cashiers</h3>
