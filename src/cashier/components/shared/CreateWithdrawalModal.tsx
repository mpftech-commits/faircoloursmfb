...
export const CreateWithdrawalModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  publicId: string;
  error?: string;
  ...
  const CreateWithdrawal = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      setLoading(true);
      const res = await api.post('/transactions/withdraw', {
       ...
       setForm({ amount: '', note: '' });
      console.log("withdrawal created succefully:", res);
    } catch (error:any) {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
              <h3 className="text-xl font-bold text-slate-900 ">Create Withdrawal</h3>
              <button 
                onClick={onClose}
            ...
            <form onSubmit={CreateWithdrawal} className="p-6 flex flex-col gap-2 overflow-y-auto">
