  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 z-20 transition-colors duration-300">
      ...
      <div className="flex items-center gap-3 md:gap-6">
        <Link to="/404">
          <button
            className="hidden lg:flex items-center gap-2 bg-blue-700 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-primary/90 transition-all shadow-sm shadow-primary/20 cursor-pointer"
          >
            <Plus size={18} />
            ...
            {showNotifications && (
              ...
              >
                ...
                <div className="space-y-3">
                  {notifications.map(n => (
                    <div key={n.id} className={`p-3 rounded-xl text-sm ${n.read ? 'bg-white' : 'bg-slate-50 border border-slate-100'}`}>
